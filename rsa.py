...
def main():
	...
	for data_file in args.data_files:
		...
		if args.nonincremental:
			...
			possible_utterances = {utterance.rstrip(' STOP') : sequence for utterance, sequence in possible_utterances.items()}
			# Set up an accumulator to hold the reformatted utterances
			reformatted_possible_utterances = dict()
			for utterance in possible_utterances:
				# Remove the STOP token from the end of the sequence
				possible_utterances[utterance]['sequence'] = [word for word in possible_utterances[utterance]['sequence'] if not 'STOP' in word.keys()]
				...
				for lex_entry in possible_utterances[utterance]['sequence']:
				...
				reformatted_possible_utterances.update({utterance : {'lexical semantics' : semantics, 'p' : p_utterances[utterance]['p']}})

			# Set the possible utterances to the reformatted ones
			possible_utterances = reformatted_possible_utterances
			...
			with open(args.output_file, 'a') as file:
				...
				for utterance in possible_utterances:
					...
					if not args.nonincremental:
						...
						file.write(f'{possible_utterances[utterance]}\n')
		...
		if args.nonincremental:
			...
			pragmatic_listener(cs, objects, verbose = verbose, output = output)
		else:
